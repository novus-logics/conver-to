...
/// Label for error or warning info.
@property (nonatomic, strong) NSString  *strFinalValue;
@property (nonatomic, weak) IBOutlet UILabel *lblFinalValue;
...
- (void)viewDidLoad
{
	...
	_dataCaptureScenarioSamples = @[
		...
		@{
			...
			RTRRegExKey : @"[0-9]+((\\.)[0-9]+)+",//@"([0-9]{2,})(\.[0-9]{2})?",
...
- (id<RTRDataCaptureService>)createConfigureDataCaptureService:(NSDictionary*)scenario
{
	...
	if([scenario[RTRRegExKey] length] == 0) {
		...
		id<RTRDataSchemeBuilder> scheme = [[builder addScheme:scenario[RTRScenarioKey]] setName:scenario[RTRScenarioKey]];
		[[[scheme addField:scenario[RTRScenarioKey]] setName:scenario[RTRScenarioKey]] setRegEx:scenario[RTRRegExKey]];
...
- (IBAction)btnDonePress:(UIButton *)sender {
    ...
    if (self.handlerBlock)
        self.handlerBlock(self.strFinalValue);
...
- (void)drawTextLine:(RTRTextLine*)textLine inLayer:(CALayer*)layer progress:(RTRResultStabilityStatus)progress
{
    ...
    self.strFinalValue = recognizedString;
    _lblFinalValue.text = recognizedString;
    NSLog(@"recognizedString: %@", recognizedString);
